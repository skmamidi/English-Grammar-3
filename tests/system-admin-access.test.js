...
test('system admin can perform operational actions only on operational resources', () => {
  ...
  assert.equal(access.canAccess(admin, access.Capabilities.manageOwnPrivacyPreferences, {
    type: access.ResourceTypes.PRIVACY_PREFERENCES,
    learnerId: 'learner-1'
  }), false);
  assert.equal(access.canAccess(admin, access.Capabilities.manageLinkedLearnerPrivacyPreferences, {
    type: access.ResourceTypes.PRIVACY_PREFERENCES,
    learnerId: 'learner-1'
  }), false);
});

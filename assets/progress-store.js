(function () {
  ...
  function updateReviewSchedules(outcomes, reviewedAt, options) {
    ...
    return updateProgress(progress => {
      ...
      progress.reviewSchedules = domain && typeof domain.applyReviewOutcomes === "function"
        ...
        : applyReviewOutcomesFallback(progress.reviewSchedules, outcomes, reviewedAt);
      return progress;
    }, options).reviewSchedules;
  }

  function applyReviewOutcomesFallback(existingSchedules, outcomes, reviewedAt) {
    ...
    (Array.isArray(outcomes) ? outcomes : []).forEach(outcome => {
      const ref = normalizeReviewScheduleQuestionRef(outcome && (outcome.questionRef || outcome.ref || outcome));
      if (!ref.id) return;
      ...
      const intervalDays = correct
        ...
        : 1;
      byId[ref.id] = normalizeReviewScheduleFallback({
        ref,
  ...
  function normalizeReviewScheduleQuestionRef(ref) {
